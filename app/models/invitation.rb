class Invitation < ActiveRecord::Base 
  ...
  validates_each :email_addresses do |record, attr, email_addresses |
    ...
    unless invalid_emails.empty?
      record.errors.add(:email_addresses, " 邮件地址有误: <ul>"+invalid_emails.collect{|email| '<li>'+email+'</li>' }.join+"</ul>")
  ...
  def send_invite
    ...
    emails.each{|email|
      UserMailer.deliver_send_invitation(email, self.user, self.message)
    }
  end
